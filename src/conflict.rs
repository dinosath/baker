...
fn apply_git_style(existing: &str, updated: &str) -> String {
    ...
    // Conflict block (only if there is an actual difference)
    if old_mid != new_mid {
    ...
    // Preserve trailing newline behaviour: if neither input ends with \n, trim last \n
...
mod tests {
    ...
    fn common_prefix_and_suffix_preserved() {
        ...
        let updated = "header\nnew line\nfooter\n";
        let result = apply_conflict_markers(existing, updated, ConflictStyle::Git);
        // Header and footer appear outside markers
