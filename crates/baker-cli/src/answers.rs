...
impl<'a> AnswerCollector<'a> {
    ...
    fn collect_question_answer(
        ...
        key: &str,
        question: &baker_core::config::Question,
    ) -> Result<()> {
        loop {
    ...
    fn validate_with_schema(
        ...
        let validator = jsonschema::validator_for(&schema_value).map_err(|e| {
            baker_core::error::Error::Other(anyhow::anyhow!("Invalid JSON schema: {}", e))
        })?;
        ...
        if !errors.is_empty() {
            return Err(baker_core::error::Error::Other(anyhow::anyhow!(
                "Validation failed: {}",
                errors.join("\n")
            )));
        }
...
mod tests {
    use super::*;
    use baker_core::config::{Type, Validation};
    use baker_core::template::get_template_engine;
    use serde_json::json;
